...
def run_naive_query(baseline: NaiveRAG, query: str, ticker: str | None = None) -> None:
...
run_baseline_query = run_naive_query


def run_interactive(baseline: NaiveRAG) -> None:
    console.print("[bold]Interactive Naive RAG mode. Type 'quit' to exit.[/bold]\n")
    while True:
        query = console.input("[bold yellow]Ask a question: [/bold yellow]")
        if query.lower() in ("quit", "exit", "q"):
            break
        if not query.strip():
            continue
        run_naive_query(baseline, query)
...
def main() -> None:
    ...
    console.print("[bold cyan]Initializing Naive RAG Pipeline...[/bold cyan]")
    ...
    console.print("[bold green]✓ Naive RAG initialized (dense-only, no reranker, no decomposition, one-shot)[/bold green]\n")

    if args.query:
        run_naive_query(baseline, args.query, ticker=args.ticker)
